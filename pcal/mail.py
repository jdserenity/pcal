"""Send a calendar event email over Proton Mail Bridge's local SMTP service."""
...
def build_invite_message(
  *,
  ...
  plain = (
    ...
    f"Open this email in Proton Mail and click Add to Proton Calendar.\n"
  )
  alt.attach(MIMEText(plain, "plain", "utf-8"))

  cal = MIMEText(ics_body, "calendar", "utf-8")
  cal.set_param("method", "PUBLISH")
  cal.set_param("name", "invite.ics")
  alt.attach(cal)
  msg.attach(alt)

  attach = MIMEBase("text", "calendar", method="PUBLISH", name="invite.ics")
  ...
  attach.set_param("method", "PUBLISH")
